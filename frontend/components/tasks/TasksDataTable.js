...
export default function TasksDataTable({ tasks, teams, onTaskClick }) {
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: 'status', header: 'Status', cell: ({ row }) => <Badge variant={statusVariant(row.original.status)}>{STATUS_LABELS[row.original.status] || row.original.status}</Badge> },
  ...
  return <TasksDataTableView columns={columns} rows={rows} onTaskClick={onTaskClick} />;
}

function TasksDataTableView({ columns, rows, onTaskClick }) {
  return <DataTable columns={columns} data={rows} searchPlaceholder="Search tasks and notes" onRowClick={onTaskClick} emptyMessage="No tasks match the current filters." />;
}
